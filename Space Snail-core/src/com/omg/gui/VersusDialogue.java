...
public class VersusDialogue extends JSActor {
	...
	public VersusDialogue() {
		...
		background = new JSActor(new TextureRegion(SpaceSnail.getAssetsManager().getTexture("VS Back"),0,0,1280,720));
		white = new JSActor(new TextureRegion(SpaceSnail.getAssetsManager().getTexture("White"),0,0,16,9));
		
	   	wind = new JSActor(new TextureRegion(SpaceSnail.getAssetsManager().getTexture("VS Swoosh"),0,0,1280,720));
		...
		wind.addAction(Actions.forever(
	   	...
	   	divideLine = new JSActor(new TextureRegion(SpaceSnail.getAssetsManager().getTexture("VS Back Border"),0,0,1280,720));
	   	vsText = new JSActor(new TextureRegion(SpaceSnail.getAssetsManager().getTexture("VS VS"),0,0,300,300));
	   	vsText.setPosition(475, 200);
	   	
	   	JSAnimation kikuAnimation = new JSAnimation("Jumping", SpaceSnail.getAssetsManager().getTexture("Kiku_Jump"), 200, 200, 10, 200);
	   	...
	   	rightComp = new JSAnimatedActor(new JSAnimation("Idle", SpaceSnail.getAssetsManager().getTexture("Enemy"), 200/2, 160/2, 10, 200));
	   	rightComp.setPosition(775, 60);
	   	rightComp.setScale(7f); //3.5f
	   	
	   	JSAnimation explode = new JSAnimation("Explode", SpaceSnail.getAssetsManager().getTexture("Explosion"), 100, 100, 4, 100);
